...
function ensureAllColumns(database) {
  ...
  ensureColumns(database, 'storyboards', [
    ...
    { name: 'spoken_text',       type: 'TEXT' },
    { name: 'offscreen_voice',   type: 'TEXT' },
    { name: 'bgm_motif',         type: 'TEXT' },
    { name: 'reference_assets',  type: 'TEXT' },
    { name: 'actual_video_model', type: 'TEXT' },
    { name: 'action',            type: 'TEXT' },
    { name: 'atmosphere',        type: 'TEXT' },
    { name: 'bgm_prompt',        type: 'TEXT' },
    ...
    { name: 'image_prompt_model',     type: 'TEXT' },
  ...
  try {
    database.exec(`CREATE TABLE IF NOT EXISTS ai_service_configs (
      ...
      credential_storage TEXT DEFAULT 'local-encrypted',
      model         TEXT,
      default_model TEXT,
      endpoint      TEXT,
      query_endpoint TEXT,
  ...
  ensureColumns(database, 'ai_service_configs', [
    ...
    { name: 'api_protocol',   type: "TEXT DEFAULT ''" },
    { name: 'api_key_ciphertext', type: 'TEXT' },
    { name: 'settings_ciphertext', type: 'TEXT' },
    { name: 'managed_group', type: 'TEXT' },
    { name: 'is_locked', type: 'INTEGER DEFAULT 0' },
    { name: 'credential_storage', type: "TEXT DEFAULT 'local-encrypted'" },
  ...
  // A reusable prop can appear in multiple episodes without duplicating its
  // generated image or moving the original props.episode_id ownership.
  ...
  // --- 商业版持久化生产流水线 ---
  database.exec(`CREATE TABLE IF NOT EXISTS preset_versions (
  ...
  database.exec(`CREATE TABLE IF NOT EXISTS pipeline_runs (
    id TEXT PRIMARY KEY,
  ...
  database.exec(`CREATE TABLE IF NOT EXISTS cloud_jobs (
    id TEXT PRIMARY KEY,
  ...
  database.exec(`CREATE TABLE IF NOT EXISTS publish_packages (
  ...
  ensureColumns(database, 'publish_packages', [
