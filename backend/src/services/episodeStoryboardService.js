...
function generateVideoPrompt(sb, style, videoRatio) {
  ...
  if (sb.dialogue) parts.push('发声要求：对白必须原句发声，说话角色嘴部自然连续运动，口型、停顿和情绪与对白同步');
  parts.push('节奏要求：镜头紧凑、动作连续，角色造型与场景保持一致，画面中不要生成字幕或文字');
  // 时长
...
async function processStoryboardGeneration(db, log, cfg, taskId, episodeId, model, style, userPrompt, systemPrompt, includeNarration, universalOmni, targetClipDurationSec = null) {
  ...
  try {
    ...
    // The mythology production already stores an approved eight-beat script
    // skeleton. If the text vendor is unavailable, turn that approved source
    // into editable storyboards instead of leaving the episode empty.
    if (/401|403|无效的令牌|No available channel|AI.*失败/i.test(String(err.message || ''))) {
      try {
        const fallback = createApprovedStoryboardFallback(db, log, episodeIdNum, cfg, streamStyle, streamVideoRatio, deriveOpts);
        if (fallback && fallback.length > 0) {
          const totalDuration = fallback.reduce((sum, sb) => sum + (Number(sb.duration) || 0), 0);
          db.prepare('UPDATE episodes SET duration = ?, updated_at = ? WHERE id = ?')
            .run(Math.ceil((totalDuration + 59) / 60), new Date().toISOString(), episodeIdNum);
          taskService.updateTaskStatus(db, taskId, 'completed', 100, '文本模型不可用，已使用已批准剧本骨架生成分镜');
          taskService.updateTaskResult(db, taskId, {
            storyboards: fallback,
            total: fallback.length,
            total_duration: totalDuration,
            duration_minutes: Math.ceil((totalDuration + 59) / 60),
            fallback: 'approved_script_skeleton',
          });
          return;
        }
      } catch (fallbackError) {
        log.error('Approved storyboard fallback failed', { error: fallbackError.message, task_id: taskId });
      }
    }
...
function createApprovedStoryboardFallback(db, log, episodeId, cfg, style, videoRatio, deriveOpts = {}) {
  const episode = db.prepare('SELECT id, drama_id, title, description, script_content FROM episodes WHERE id = ? AND deleted_at IS NULL').get(Number(episodeId));
  if (!episode) return [];
  const metadataRow = db.prepare('SELECT metadata FROM dramas WHERE id = ? AND deleted_at IS NULL').get(episode.drama_id);
  let metadata = {};
  try { metadata = JSON.parse(metadataRow?.metadata || '{}'); } catch (_) {}
  if (metadata.preset_key !== 'landscape_chinese_myth_v1') return [];

  const scenes = db.prepare('SELECT id, location, time FROM scenes WHERE episode_id = ? AND deleted_at IS NULL ORDER BY id ASC').all(episode.id);
  if (!scenes.length) return [];
  const characterIds = db.prepare('SELECT character_id FROM episode_characters WHERE episode_id = ?').all(episode.id).map((row) => Number(row.character_id));
  const propIds = db.prepare(
    `SELECT DISTINCT p.id FROM props p
     LEFT JOIN episode_props epm ON epm.prop_id = p.id AND epm.episode_id = ?
     WHERE p.deleted_at IS NULL AND (p.episode_id = ? OR epm.episode_id IS NOT NULL)`
  ).all(episode.id, episode.id).map((row) => Number(row.id));
  const script = String(episode.script_content || episode.description || '');
  const beatLines = [...script.matchAll(/镜头\s*(\d+)[^：:]*[：:]\s*([^\n]+)/g)]
    .sort((a, b) => Number(a[1]) - Number(b[1]))
    .map((match) => match[2].trim());
  const count = Number(metadata.storyboard_count || metadata.clip_count || 8);
  const duration = Number(metadata.video_clip_duration || metadata.clip_duration || deriveOpts.targetClipDuration || 10);
  const beats = Array.from({ length: Math.max(1, Math.min(20, count)) }, (_, index) =>
    beatLines[index] || `围绕“${episode.title}”推进一个清晰动作，镜头保持电影感横屏构图并为下一镜留下动作结果。`
  );
  const boards = beats.map((action, index) => {
    const scene = scenes[index % scenes.length];
    const characterText = characterIds.length ? `角色参考 ID：${characterIds.join('、')}。` : '无角色开口。';
    const propText = propIds.length ? `道具参考 ID：${propIds.join('、')}。` : '';
    return {
      shot_number: index + 1,
      title: `${episode.title}｜镜头${index + 1}`,
      location: scene.location,
      time: scene.time,
      duration,
      action: `${action} ${characterText}${propText}`,
      result: index === beats.length - 1 ? '留下通往下一镜或下一集的视觉钩子。' : '动作结果清晰衔接下一镜。',
      atmosphere: '史诗、克制、紧凑、电影感、家庭向',
      shot_type: index === 0 ? '远景建立' : '中近景动作镜头',
      movement: index % 2 === 0 ? '缓推后快速跟随' : '横移衔接并轻微上摇',
      characters: characterIds,
      props: propIds,
      image_prompt: `Cinematic Chinese mythology, widescreen ${videoRatio}, ${scene.location}, ${action}, no text, no subtitles, no watermark.`,
      creation_mode: 'classic',
    };
  });
  const saved = saveStoryboards(db, log, episode.id, boards, cfg, style, null, deriveOpts);
  for (const sb of saved) syncStoryboardCharacters(db, log, sb.id);
  log.warn('Storyboard generation used approved script skeleton fallback', { episode_id: episode.id, count: saved.length });
  return saved;
}
