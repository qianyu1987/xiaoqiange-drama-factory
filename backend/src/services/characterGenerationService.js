...
async function processCharacterGeneration(db, cfg, log, taskID, req) {
  ...
  try {
    ...
    // A locked project may already contain approved character sheets. Keep the
    // extraction button usable when the text vendor is temporarily unavailable.
    // This fallback only links existing catalog rows; it never invents a new
    // character or claims that an AI extraction succeeded.
    const fallback = linkExistingCatalogCharacters(db, log, req, outlineText, err.message);
    if (fallback) {
      taskService.updateTaskStatus(db, taskID, 'completed', 100, '文本模型不可用，已使用本剧已锁定角色库完成关联');
      taskService.updateTaskResult(db, taskID, fallback);
      return;
    }
...
/**
 * Deterministic fallback for approved productions. It is deliberately narrow:
 * only existing characters whose names occur in the current script (or are
 * already linked to the episode) are returned, so no hallucinated cast enters
 * a customer project when the text provider is down.
 */
function linkExistingCatalogCharacters(db, log, req, outlineText, reason) {
  const dramaId = Number(req.drama_id);
  const episodeId = req.episode_id != null ? Number(req.episode_id) : null;
  if (!Number.isFinite(dramaId)) return null;
  const rows = db.prepare(
    'SELECT id, drama_id, name, role, description, personality, appearance, voice_style FROM characters WHERE drama_id = ? AND deleted_at IS NULL ORDER BY sort_order ASC, id ASC'
  ).all(dramaId);
  if (!rows.length) return null;
  const script = String(outlineText || '');
  const linked = episodeId
    ? new Set(db.prepare('SELECT character_id FROM episode_characters WHERE episode_id = ?').all(episodeId).map((row) => Number(row.character_id)))
    : new Set();
  const selected = rows.filter((row) => linked.has(row.id) || (row.name && script.includes(row.name)));
  if (!selected.length) return null;
  if (episodeId) {
    const insert = db.prepare('INSERT OR IGNORE INTO episode_characters (episode_id, character_id) VALUES (?, ?)');
    for (const row of selected) insert.run(episodeId, row.id);
  }
  log.warn('Character extraction used approved catalog fallback', {
    drama_id: dramaId,
    episode_id: episodeId,
    character_count: selected.length,
    provider_error: String(reason || '').slice(0, 200),
  });
  return { characters: selected, count: selected.length, fallback: 'approved_catalog', provider_error: String(reason || '').slice(0, 200) };
}
