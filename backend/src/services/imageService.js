...
function create(db, log, req) {
  ...
  if (req.reference_images && Array.isArray(req.reference_images)) {
    log.info('reference_images 完整路径（请求入参）', {
      ...
      reference_files: req.reference_images.map((item) => path.basename(String(item || ''))),
    });
  }
...
async function processImageGeneration(db, log, imageGenId) {
  ...
  try {
    ...
    // ── Step 1: 获取 AI 配置 ──────────────────────────────────────────
    const dramaMetadata = row.drama_id
      ? db.prepare('SELECT metadata FROM dramas WHERE id = ? AND deleted_at IS NULL').get(row.drama_id)
      : null;
    let isChineseMythology = false;
    try {
      const metadata = JSON.parse(dramaMetadata?.metadata || '{}');
      isChineseMythology = metadata.production_slug === 'primordial-chinese-myth' || metadata.preset_key === 'landscape_chinese_myth_v1';
    } catch (_) {}
    const preferredPrivateProvider = !isChineseMythology && ['yyapi_private_direct', 'hhtc_private_direct'].includes(row.provider)
      ? row.provider
      : null;
    const config = imageClient.getDefaultImageConfig(db, row.model, preferredPrivateProvider, imageServiceType);
    if (!config) {
    ...
    log.info('[图生] Step2 参考图', {
      ...
      reference_files: (reference_image_urls || []).map((item) => path.basename(String(item || ''))),
      elapsed: elapsed(),
    });
    ...
    const result = await imageClient.callImageApi(db, log, {
      prompt: finalPrompt,
      model: row.model,
      preferred_provider: preferredPrivateProvider || undefined,
      size: imageSize,
      quality: row.quality,
      drama_id: row.drama_id,
    ...
    let localPath = null;
    let privateFactoryRelativePath = null;
    try {
      const defaultStoragePath = path.isAbsolute(cfg.storage?.local_path)
        ? cfg.storage.local_path
        : path.join(process.cwd(), cfg.storage?.local_path || './data/storage');
      const isPrivateDirect = result.private_direct === true;
      const storagePath = isPrivateDirect ? imageClient.PRIVATE_FACTORY_ROOT : defaultStoragePath;
      const category = isPrivateDirect
        ? 'private-yyapi/scene-masters'
        : (row.scene_id != null ? 'scenes' : row.character_id != null ? 'characters' : 'images');
      const projectSubdir = isPrivateDirect ? null : storageLayout.getProjectStorageSubdir(db, row.drama_id);
      const savedPath = await uploadService.downloadImageToLocal(
        storagePath,
        result.image_url,
        category,
        log,
      ...
      localPath = isPrivateDirect && savedPath ? path.join(storagePath, savedPath) : savedPath;
      privateFactoryRelativePath = isPrivateDirect ? savedPath : null;
      if (localPath && imageSize) {
        const absImg = isPrivateDirect ? localPath : path.join(storagePath, localPath);
    ...
    if (localPath) {
      persistedImageUrl = privateFactoryRelativePath
        ? '/factory-media/' + String(privateFactoryRelativePath).replace(/^\//, '')
