...
function inferVideoProtocol(provider) {
  ...
  if (p === 'agnes' || p === 'agnes_video') return 'agnes_video';
  if (p === 'hhtc' || p === 'hhtc_video_generation') return 'hhtc_video_generation';
...
const HHTC_VIDEO_ALLOWED_IMAGE_TYPES = new Set(['image/png', 'image/jpeg', 'image/webp']);
const HHTC_VIDEO_MAX_REFERENCE_IMAGES = 2;

function hhtcVideoBase(config) {
  return String(config.base_url || 'https://www.hhtc.top')
    .replace(/\/$/, '')
    .replace(/\/v1$/i, '');
}

function hhtcVideoHeaders(config) {
  return { Authorization: `Bearer ${String(config.api_key || process.env.HHTC_APP_ACCESS_TOKEN || '').trim()}` };
}

function hhtcVideoEndpoint(base, suffix) {
  if (/\/model-gateway\/video$/i.test(base)) return `${base}${suffix}`;
  return `${base}/pay/api/video-generation${suffix}`;
}

function hhtcVideoError(response, fallback) {
  const detail = response?.error?.message || response?.error || response?.message || fallback;
  return String(detail || fallback).slice(0, 500);
}

function localImageDataUrlForHhtc(raw, storageLocalPath) {
  const source = String(raw || '').trim();
  if (!source) return { error: '参考图为空' };
  const dataMatch = source.match(/^data:(image\/(?:png|jpeg|webp));base64,([A-Za-z0-9+/=\s]+)$/i);
  if (dataMatch) {
    const buffer = Buffer.from(dataMatch[2].replace(/\s+/g, ''), 'base64');
    if (!buffer.length) return { error: '参考图内容为空' };
    if (buffer.length > 8 * 1024 * 1024) return { error: '单张参考图不能超过 8MB' };
    return { dataUrl: `data:${dataMatch[1].toLowerCase()};base64,${buffer.toString('base64')}` };
  }

  let filePath = source;
  if (/^https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?\//i.test(source)) {
    const relative = source.split('/static/')[1];
    if (!relative || !storageLocalPath) return { error: '本地参考图路径无法解析' };
    const root = path.resolve(storageLocalPath);
    filePath = path.resolve(root, decodeURIComponent(relative).replace(/^\/+/, ''));
    if (!filePath.startsWith(root + path.sep)) return { error: '本地参考图路径无效' };
  }
  if (!path.isAbsolute(filePath)) return { error: 'HHTC 视频仅接受本地或 data URL 参考图' };
  try {
    ...
    if (!HHTC_VIDEO_ALLOWED_IMAGE_TYPES.has(mime)) return { error: '参考图仅支持 PNG、JPG 或 WebP' };
    return { dataUrl: `data:${mime};base64,${buffer.toString('base64')}` };
  } catch (_) {
    return { error: '本地参考图无法读取' };
  }
}

function hhtcVideoReferenceInputs(opts) {
  const candidates = [opts.image_url, opts.first_frame_url, ...(Array.isArray(opts.reference_urls) ? opts.reference_urls : [])]
    .map((item) => String(item || '').trim())
    .filter(Boolean);
  const unique = [...new Set(candidates)].slice(0, HHTC_VIDEO_MAX_REFERENCE_IMAGES);
  const dataUrls = [];
  for (const source of unique) {
    const result = localImageDataUrlForHhtc(source, opts.storage_local_path);
...
async function hhtcJsonRequest(url, config, options = {}) {
  const response = await fetch(url, {
    ...options,
    headers: {
      ...hhtcVideoHeaders(config),
      ...(options.headers || {}),
    },
  });
...
async function callHhtcVideoGenerationApi(config, log, opts) {
  const base = hhtcVideoBase(config);
  const references = hhtcVideoReferenceInputs(opts);
  if (references.error) return { error: references.error };
  const assetIds = [];
  for (let index = 0; index < references.dataUrls.length; index++) {
    const uploaded = await hhtcJsonRequest(hhtcVideoEndpoint(base, '/assets'), config, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        filename: `local-reference-${index + 1}.png`,
        dataUrl: references.dataUrls[index],
      }),
    });
    if (!uploaded.response.ok || !uploaded.body?.success || !uploaded.body?.data?.asset?.assetId) {
      return { error: hhtcVideoError(uploaded.body, `HHTC 参考图上传失败 (${uploaded.response.status})`) };
    }
    assetIds.push(String(uploaded.body.data.asset.assetId));
  }
  const requestedDuration = Number(opts.duration) || 5;
  const duration = [5, 10, 15].includes(requestedDuration) ? requestedDuration : requestedDuration <= 7 ? 5 : requestedDuration <= 12 ? 10 : 15;
  const aspectRatio = ['16:9', '9:16', '1:1', '4:3', '3:4', '21:9'].includes(String(opts.aspect_ratio || ''))
    ? String(opts.aspect_ratio)
    : '16:9';
  const submitted = await hhtcJsonRequest(hhtcVideoEndpoint(base, '/jobs'), config, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Idempotency-Key': `local-mini-drama-${String(opts.video_gen_id || Date.now())}`,
    },
    body: JSON.stringify({
      model: opts.model,
  ...
  if (!submitted.response.ok || !submitted.body?.success || !submitted.body?.data?.job?.jobNo) {
    return { error: hhtcVideoError(submitted.body, `HHTC 视频任务提交失败 (${submitted.response.status})`) };
  }
  const jobNo = String(submitted.body.data.job.jobNo);
  log.info('[HHTC 视频] 已提交', { video_gen_id: opts.video_gen_id, job_no: jobNo, asset_count: assetIds.length, model: opts.model });
  return { task_id: `hhtc:${jobNo}`, status: String(submitted.body.data.job.status || 'queued') };
}

async function pollHhtcVideoGenerationJob(config, log, videoGenId, taskId) {
  const jobNo = String(taskId || '').replace(/^hhtc:/, '').trim();
  if (!jobNo) return { error: 'HHTC 视频任务编号为空' };
  const base = hhtcVideoBase(config);
  const result = await hhtcJsonRequest(hhtcVideoEndpoint(base, `/jobs/${encodeURIComponent(jobNo)}`), config, { method: 'GET' });
  if (!result.response.ok || !result.body?.success || !result.body?.data?.job) {
    return { error: hhtcVideoError(result.body, `HHTC 视频任务查询失败 (${result.response.status})`) };
  }
  const job = result.body.data.job;
  const status = String(job.status || '').toLowerCase();
  log.info('[HHTC 视频] 任务状态', { video_gen_id: videoGenId, job_no: jobNo, status, progress: Number(job.progress || 0) });
  if (status === 'completed' && job.ready && job.downloadUrl) {
    return {
      video_url: `${base}${String(job.downloadUrl)}`,
      download_headers: hhtcVideoHeaders(config),
    };
  }
  if (['failed', 'cancelled', 'canceled'].includes(status)) {
    return { error: String(job.error || 'HHTC 视频任务失败').slice(0, 500) };
  }
  return { pending: true };
}

function agnesVideoBase(config) {
  return String(config.base_url || 'https://apihub.agnes-ai.com')
    .replace(/\/$/, '')
    .replace(/\/v1$/i, '');
}

function agnesVideoSize(aspectRatio) {
  const sizes = {
  ...
  return sizes[String(aspectRatio || '')] || sizes['16:9'];
}

function agnesVideoFrames(duration) {
  const seconds = Math.max(1, Math.min(18, Number(duration) || 5));
  return Math.min(441, Math.round((seconds * 24 - 1) / 8) * 8 + 1);
}

const AGNES_25_ALIASES = new Set(['agnes-video-2.5', 'agnes-video-v2.5']);
const AGNES_25_MODEL = 'agnes-video-2.5';

async function listAgnesVideoModels(config) {
  const response = await fetch(`${agnesVideoBase(config)}/v1/models`, {
  ...
  if (!response.ok || !data) {
    throw Object.assign(new Error(`Agnes 模型列表查询失败 (${response.status})`), { code: 'AGNES_MODEL_DISCOVERY_FAILED' });
  }
  const rows = Array.isArray(data.data) ? data.data : Array.isArray(data.models) ? data.models : [];
  return rows.map((item) => String(typeof item === 'string' ? item : item?.id || item?.name || '').trim()).filter(Boolean);
}

async function resolveAgnes25Model(config, requestedModel) {
  const requested = String(requestedModel || '').trim().toLowerCase();
  if (!AGNES_25_ALIASES.has(requested)) return String(requestedModel || '').trim();
  // Agnes 2.5 is a documented preview model and may not be returned by
  // /v1/models. The create endpoint is the authority for account access.
  return AGNES_25_MODEL;
}

function encodeAgnesTaskId(videoId, model) {
  if (!AGNES_25_ALIASES.has(String(model || '').toLowerCase())) return `agnes:${videoId}`;
  return `agnes:${encodeURIComponent(model)}:${videoId}`;
}

function decodeAgnesTaskId(taskId) {
  const raw = String(taskId || '').replace(/^agnes:/, '').trim();
  const separator = raw.indexOf(':');
  if (separator < 0) return { videoId: raw, model: 'agnes-video-v2.0' };
  return {
    model: decodeURIComponent(raw.slice(0, separator)),
    videoId: raw.slice(separator + 1),
  };
}

async function publicImageUrlForAgnes(raw, storageLocalPath, log, tag) {
  const source = String(raw || '').trim();
  if (!source) return null;
  if (/^https?:\/\//i.test(source) && !/^https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?\//i.test(source)) {
    return source;
  }
  const dataMatch = source.match(/^data:(image\/(?:png|jpeg|webp));base64,(.+)$/i);
  if (dataMatch) {
    return uploadToImageProxy(Buffer.from(dataMatch[2], 'base64'), dataMatch[1].toLowerCase(), log, tag);
  }
  if (/^https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?\/factory-media\//i.test(source)) {
    const factoryRoot = String(process.env.PRIVATE_FACTORY_ROOT || '').trim();
    if (!factoryRoot) return null;
    const relative = decodeURIComponent(source.split('/factory-media/')[1] || '').replace(/^\/+/, '');
    const factoryPath = path.join(factoryRoot, relative);
    return uploadLocalImageToProxy(storageLocalPath, factoryPath, log, tag);
  }
  return uploadLocalImageToProxy(storageLocalPath, source, log, tag);
}

async function callAgnesVideoApi(config, log, opts) {
  let actualModel;
  try {
    actualModel = await resolveAgnes25Model(config, opts.model || 'agnes-video-v2.0');
  } catch (error) {
    return { error: error.message, code: error.code || 'AGNES_MODEL_DISCOVERY_FAILED' };
  }
  const isAgnes25 = AGNES_25_ALIASES.has(String(actualModel || '').toLowerCase());
  const firstFrameSource = String(opts.first_frame_url || '').trim();
  const lastFrameSource = String(opts.last_frame_url || '').trim();
  const referenceSources = [
    opts.image_url,
    ...(Array.isArray(opts.reference_urls) ? opts.reference_urls : []),
  ].map((item) => String(item || '').trim()).filter(Boolean);
  const candidates = isAgnes25 && (firstFrameSource || lastFrameSource)
    ? [firstFrameSource, lastFrameSource].filter(Boolean)
    : isAgnes25
      ? referenceSources
      : [opts.image_url, firstFrameSource, ...referenceSources, lastFrameSource]
          .map((item) => String(item || '').trim()).filter(Boolean);
  const references = [];
  const referenceLimit = isAgnes25 ? 10 : 2;
  for (const source of [...new Set(candidates)].slice(0, referenceLimit)) {
    const url = await publicImageUrlForAgnes(
      source,
      opts.storage_local_path,
      log,
      `agnes_vg${opts.video_gen_id}_${references.length + 1}`
    );
    if (!url) return { error: 'Agnes 参考图无法转换为公网 URL' };
    references.push(url);
  }
  ...
  if (isAgnes25) {
    body = {
      ...
      prompt: String(opts.prompt || ''),
      seconds: Math.max(4, Math.min(12, Number(opts.duration) || 5)),
      mode: 'text',
      size: '720P',
    ...
    if (firstFrameSource || lastFrameSource) {
      body.mode = 'keyframe';
      let index = 0;
      if (firstFrameSource) body.first_frame = references[index++];
      if (lastFrameSource) body.last_frame = references[index];
    } else if (references.length > 0) {
      body.mode = 'reference';
      body.images = references;
    }
  } else {
    const size = agnesVideoSize(opts.aspect_ratio);
    body = {
      model: actualModel || 'agnes-video-v2.0',
      prompt: String(opts.prompt || ''),
      width: size.width,
      height: size.height,
      num_frames: agnesVideoFrames(opts.duration),
      frame_rate: 24,
    };
    if (opts.seed != null) body.seed = Number(opts.seed);
  ...
  const response = await fetch(`${agnesVideoBase(config)}/v1/videos`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  try { data = raw ? JSON.parse(raw) : null; } catch (_) {}
  if (!response.ok || !data) {
    return { error: hhtcVideoError(data, `Agnes 视频任务提交失败 (${response.status})`) };
  }
  const videoId = String(data.video_id || data.task_id || data.id || '').trim();
  if (!videoId) return { error: 'Agnes 未返回 video_id 或 task_id' };
  log.info('[Agnes 视频] 已提交', {
  ...
  const submitted = {
    task_id: encodeAgnesTaskId(videoId, actualModel),
    status: String(data.status || 'queued'),
  };
  if (AGNES_25_ALIASES.has(String(actualModel || '').toLowerCase())) submitted.actual_model = actualModel;
  return submitted;
}

async function pollAgnesVideoJob(config, log, videoGenId, taskId) {
  const decoded = decodeAgnesTaskId(taskId);
  const videoId = decoded.videoId;
  if (!videoId) return { error: 'Agnes video_id 为空' };
  const isAgnes25 = AGNES_25_ALIASES.has(String(decoded.model || '').toLowerCase());
  const url = isAgnes25
    ? new URL(`${agnesVideoBase(config)}/v1/videos/${encodeURIComponent(videoId)}`)
    : new URL(`${agnesVideoBase(config)}/agnesapi`);
  if (!isAgnes25) {
    url.searchParams.set('video_id', videoId);
    url.searchParams.set('model_name', decoded.model);
  }
  const response = await fetch(url, {
    headers: { Authorization: `Bearer ${String(config.api_key || '').trim()}` },
  });
  ...
  let data = null;
  try { data = raw ? JSON.parse(raw) : null; } catch (_) {}
  if (response.status === 429 || /rate\s*limit/i.test(String(data?.error?.message || data?.error || data?.message || ''))) {
    log.warn('[Agnes 视频] 查询限流，保留任务并继续轮询', {
  ...
  if (!response.ok || !data) {
    return { error: hhtcVideoError(data, `Agnes 视频任务查询失败 (${response.status})`) };
  }
  const status = String(data.status || '').toLowerCase();
  const videoUrl = data?.url || data?.metadata?.url || pickProxyVideoUrl(data);
  log.info('[Agnes 视频] 任务状态', {
    video_gen_id: videoGenId,
    video_id: videoId,
    status,
    progress: Number(data.progress || 0),
  });
  if (status === 'completed' && isPlausibleHttpVideoUrl(videoUrl)) return { video_url: videoUrl };
  if (status === 'failed') {
    return { error: String(data?.error?.message || data?.error || 'Agnes 视频生成失败').slice(0, 500) };
  }
  return { pending: true };
}
...
async function callVideoApi(db, log, opts) {
  ...
  if (protocol === 'hhtc_video_generation') {
    return callHhtcVideoGenerationApi(config, log, {
      prompt,
      model,
      duration: opts.duration,
      aspect_ratio,
  ...
  if (protocol === 'agnes_video') {
    return callAgnesVideoApi(config, log, {
      prompt,
...
async function pollVideoTask(db, log, videoGenId, taskId, config, maxAttempts = 180, intervalMs = 10000) {
  ...
  const isHhtcVideo = protocol === 'hhtc_video_generation' || (typeof taskId === 'string' && taskId.startsWith('hhtc:'));
  const isAgnesVideo = protocol === 'agnes_video' || (typeof taskId === 'string' && taskId.startsWith('agnes:'));
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (isKling) {
        ...
        const hhtc = await pollHhtcVideoGenerationJob(config, log, videoGenId, taskId);
        if (hhtc.video_url || hhtc.error) return hhtc;
        continue;
      } else if (isAgnesVideo) {
        const agnes = await pollAgnesVideoJob(config, log, videoGenId, taskId);
        if (agnes.video_url || agnes.error) return agnes;
        continue;
      } else if (isGemini) {
        const base = (config.base_url || 'https://generativelanguage.googleapis.com').replace(/\/$/, '');
...
module.exports = {
  ...
  callVideoApi,
  pollVideoTask,
  callHhtcVideoGenerationApi,
  pollHhtcVideoGenerationJob,
  callAgnesVideoApi,
  pollAgnesVideoJob,
  listAgnesVideoModels,
  resolveAgnes25Model,
