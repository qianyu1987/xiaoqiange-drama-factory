...
async function generateScenePromptOnly(db, log, cfg, sceneId, modelName, style) {
  ...
  try {
    generated = await generateTextWithFallback(db, log, {
      ...
      userPrompt,
      systemPrompt,
      options: { scene_key: 'scene_image_polish', max_tokens: 4000 },
    });
  } catch (err) {
  ...
  db.prepare('UPDATE scenes SET polished_prompt = ?, image_prompt_model = ?, updated_at = ? WHERE id = ?').run(
    polishedPrompt, generated.model, new Date().toISOString(), Number(sceneId)
  );
