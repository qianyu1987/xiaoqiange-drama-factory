...
async function processBackgroundExtraction(db, cfg, log, taskID, episodeId, model, style, language) {
  ...
  try {
    ...
    if (existingScenes.length > 0) {
      log.warn('Background extraction used approved catalog fallback', {
        episode_id: Number(episodeId),
        scene_count: existingScenes.length,
        provider_error: String(err.message || '').slice(0, 200),
      });
      taskService.updateTaskStatus(db, taskID, 'completed', 100, '文本模型不可用，已保留本集已锁定场景');
