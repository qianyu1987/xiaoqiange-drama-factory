...
const { generateTextWithFallback } = require('./textModelFallbackService');
...
async function generateCharacterPromptOnly(db, log, cfg, characterId, modelName, style) {
  ...
  let projectMeta = {};
  try { projectMeta = JSON.parse(dramaFull?.metadata || '{}'); } catch (_) {}
  const promptModels = modelName
    ? [modelName]
    : Array.isArray(projectMeta.image_prompt_models) && projectMeta.image_prompt_models.length
      ? projectMeta.image_prompt_models
      : [];
  let generated;
  try {
    generated = await generateTextWithFallback(db, log, {
      models: promptModels,
      userPrompt,
      systemPrompt,
      options: { scene_key: 'role_image_polish', max_tokens: 4000 },
    });
  } catch (err) {
    log.error('[四视图提示词] 文本AI全部失败', { error: err.message });
    return { ok: false, error: err.message };
  }
  const fourViewDescription = generated.text;
  ...
  db.prepare('UPDATE characters SET polished_prompt = ?, image_prompt_model = ?, updated_at = ? WHERE id = ?').run(
    polishedPrompt, generated.model, new Date().toISOString(), Number(characterId)
  );

  log.info('[四视图提示词] 生成并保存完成', { character_id: characterId, length: polishedPrompt.length });
  return { ok: true, polished_prompt: polishedPrompt, image_prompt_model: generated.model, fallback_from: generated.fallback_from };
}
