...
function createStoryboard(db, log, req) {
  ...
  const info = db.prepare(
    `INSERT INTO storyboards (episode_id, scene_id, storyboard_number, title, description, location, time, duration, dialogue, action, result, atmosphere, bgm_prompt, sound_effect, image_prompt, video_prompt, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)`
  ).run(
    episodeId,
    req.scene_id ?? null,
...
function updateStoryboard(db, log, id, req) {
  ...
  if (!row) return null;
  const allowed = ['title', 'description', 'location', 'time', 'duration', 'dialogue', 'narration', 'speaker', 'spoken_text', 'offscreen_voice', 'bgm_motif', 'reference_assets', 'actual_video_model', 'action', 'result', 'atmosphere', 'bgm_prompt', 'sound_effect', 'image_prompt', 'polished_prompt', 'video_prompt', 'scene_id', 'characters', 'composed_image', 'image_url', 'local_path', 'main_panel_idx', 'video_url', 'audio_local_path', 'narration_audio_local_path', 'status', 'shot_type', 'angle', 'angle_h', 'angle_v', 'angle_s', 'movement', 'segment_index', 'segment_title', 'creation_mode', 'universal_segment_text'];
  const updates = [];
  const params = [];
...
function getStoryboardById(db, id) {
  ...
  return {
    ...
    reference_assets: r.reference_assets ? (() => { try { return JSON.parse(r.reference_assets); } catch (_) { return []; } })() : [],
    actual_video_model: r.actual_video_model ?? null,
    image_prompt_model: r.image_prompt_model ?? null,
    action: r.action,
    result: r.result ?? null,
    atmosphere: r.atmosphere,
    bgm_prompt: r.bgm_prompt ?? null,
    sound_effect: r.sound_effect ?? null,
