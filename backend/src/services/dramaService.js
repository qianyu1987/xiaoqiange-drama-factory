...
function getDrama(db, dramaId, baseUrl) {
  ...
  for (const ep of drama.episodes) {
    ...
    try {
      ...
      const bySharedEpisode = db.prepare(
        `SELECT p.* FROM props p
         INNER JOIN episode_props epm ON epm.prop_id = p.id
         WHERE epm.episode_id = ? AND p.deleted_at IS NULL ORDER BY p.id ASC`
      ).all(ep.id);
      const seen = new Set();
      ep.props = [];
      ...
      for (const p of bySharedEpisode) {
        if (!seen.has(p.id)) {
          seen.add(p.id);
          ep.props.push(rowToProp(p));
        }
...
function rowToStoryboard(r) {
  return {
    ...
    spoken_text: r.spoken_text ?? null,
    offscreen_voice: r.offscreen_voice ?? null,
    bgm_motif: r.bgm_motif ?? null,
    reference_assets: parseJsonColumn(r.reference_assets),
    actual_video_model: r.actual_video_model ?? null,
    image_prompt_model: r.image_prompt_model ?? null,
    action: r.action,
...
function saveEpisodes(db, log, dramaId, req) {
  ...
  const liveCount = db.prepare(
    'SELECT COUNT(*) AS cnt FROM episodes WHERE drama_id = ? AND deleted_at IS NULL'
  ).get(did).cnt;
  db.prepare('UPDATE dramas SET total_episodes = ?, updated_at = ? WHERE id = ?').run(liveCount, now, did);
  log.info('Episodes saved', { drama_id: dramaId, count: episodes.length, total_episodes: liveCount });
  return true;
}
...
function finalizeEpisode(db, log, episodeId, baseUrl, body = {}) {
  ...
  const dramaMetadata = parseJsonColumn(drama?.metadata) || {};
  const preserveNativeAudio = dramaMetadata.native_audio_required === true || dramaMetadata.preset_key === 'landscape_chinese_myth_v1';
  const storyboards = db.prepare(
  ...
  const mergeReq = {
    ...
    provider: 'ffmpeg',
    merge_options: {
      burn_narration_subtitles: preserveNativeAudio ? false : !!(body && body.burn_narration_subtitles),
      burn_dialogue_audio: preserveNativeAudio ? false : !!(body && body.burn_dialogue_audio),
