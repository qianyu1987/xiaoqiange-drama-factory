...
async function uploadLocalImageToProxy(storagePath, localPathOrUrl, log, tag) {
  try {
    ...
    const rawPath = String(localPathOrUrl || '').trim();
    if (rawPath.startsWith('http')) {
      // localhost URL → 提取 /static/ 后的相对路径
      const afterStatic = rawPath.split('/static/')[1];
      if (afterStatic && storagePath) {
        filePath = path.join(storagePath, decodeURIComponent(afterStatic.replace(/^\//, '')));
      }
    } else if (rawPath && storagePath) {
      // The API commonly stores local assets as either `projects/...` or
      // `/static/projects/...`; normalize both forms before joining storage.
      const relative = rawPath.replace(/^\/static\//i, '').replace(/^\//, '');
      filePath = path.isAbsolute(rawPath) && !/^\/static\//i.test(rawPath)
        ? rawPath
        : path.join(storagePath, decodeURIComponent(relative));
    }
