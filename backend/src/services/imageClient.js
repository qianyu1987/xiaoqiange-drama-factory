...
function inferProtocol(provider, model) {
  ...
  if (p === 'agnes' && /^agnes-image-/i.test(model || '')) return 'agnes_image';
...
function agnesImageRequestSize(size) {
  const raw = String(size || '').trim();
  if (/^[1-4]K$/i.test(raw)) return { size: raw.toUpperCase(), ratio: '1:1' };
  if (AGNES_IMAGE_RATIOS.some(([ratio]) => ratio === raw)) return { size: '1K', ratio: raw };

  const match = raw.toLowerCase().replace('*', 'x').match(/^(\d+)\s*x\s*(\d+)$/);
  if (!match) return { size: '1K', ratio: '1:1' };
  const requestedRatio = Number(match[1]) / Number(match[2]);
  const [ratio] = AGNES_IMAGE_RATIOS.reduce((best, candidate) => (
    Math.abs(candidate[1] - requestedRatio) < Math.abs(best[1] - requestedRatio) ? candidate : best
  ));
...
/**
 * Resolve exactly one private reference image for the direct YYAPI edit path.
 * This intentionally rejects data URLs and remote URLs: the only upload is the
 * multipart request from this Mac straight to the configured provider.
 */
function resolvePrivateDirectReference(value, filesBaseUrl, storageLocalPath) {
  if (!value || !String(value).trim()) throw new Error('私密直传需要一张本地参考图');
  const raw = String(value).trim();
  let filePath = null;

  if (raw.startsWith('http://') || raw.startsWith('https://')) {
    if (!/localhost|127\.0\.0\.1/i.test(raw) || !storageLocalPath) {
      throw new Error('私密直传拒绝远程参考图，只接受本地已选素材');
    }
    const base = (filesBaseUrl || '').replace(/\/$/, '');
    const relative = raw.split('/static/')[1]
      || (base && raw.startsWith(base + '/') ? raw.slice(base.length + 1) : null);
    if (!relative) throw new Error('私密直传无法解析本地参考图路径');
    filePath = path.resolve(storageLocalPath, relative.replace(/^\//, ''));
  } else if (path.isAbsolute(raw)) {
    filePath = path.resolve(raw);
  } else if (storageLocalPath) {
    filePath = path.resolve(storageLocalPath, raw.replace(/^\//, ''));
  }

  if (!filePath) throw new Error('私密直传需要可读取的本地参考图');
  const allowedRoots = PRIVATE_FACTORY_ROOT ? [PRIVATE_FACTORY_ROOT] : [];
  if (storageLocalPath) allowedRoots.push(path.resolve(storageLocalPath));
  if (!allowedRoots.some((root) => isPathInside(root, filePath))) {
    throw new Error('私密直传参考图不在允许的本地素材目录中');
  }
  if (!fs.existsSync(filePath)) throw new Error('私密直传参考图文件不存在');
  const stat = fs.statSync(filePath);
  if (!stat.isFile()) throw new Error('私密直传参考图不是文件');
  if (stat.size > PRIVATE_DIRECT_MAX_REFERENCE_BYTES) {
    throw new Error('私密直传参考图超过 20MB 限制');
  }
  const ext = path.extname(filePath).toLowerCase();
  const mimeType = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp' }[ext];
  if (!mimeType) throw new Error('私密直传只接受 PNG、JPG 或 WebP 参考图');
...
function postMultipartWithTimeout(url, headers, body, timeoutMs = IMAGE_HTTP_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const request = transport.request({
      ...
      response.on('end', () => {
        clearTimeout(timer);
        resolve({ statusCode: response.statusCode || 0, raw: Buffer.concat(chunks).toString('utf8') });
      });
      response.on('error', (error) => { clearTimeout(timer); reject(error); });
    });
    const timer = setTimeout(() => {
      request.destroy();
      reject(new Error(`Private image edit HTTP timeout after ${timeoutMs}ms`));
    }, timeoutMs);
...
function providerErrorMessage(raw, statusCode) {
  try {
    const parsed = JSON.parse(raw || '{}');
    const message = parsed?.error?.message || parsed?.message || parsed?.error?.code || parsed?.code;
    return message ? String(message).slice(0, 220) : `HTTP ${statusCode}`;
  } catch (_) {
...
async function callOpenAIImageEditDirect(config, log, opts) {
  const refs = Array.isArray(opts.reference_image_urls) ? opts.reference_image_urls.filter(Boolean) : [];
  if (refs.length !== 1) return { error: '私密直传每次只能提交一张已选参考图' };

  let reference;
  try {
    reference = resolvePrivateDirectReference(refs[0], opts.files_base_url, opts.storage_local_path);
  } catch (error) {
    return { error: error.message };
  }

  const base = String(config.base_url || '').replace(/\/$/, '');
  if (!base) return { error: '私密直传缺少 YYAPI Base URL' };
  // A legacy OpenAI config may retain its auto-filled generations endpoint.
  // This protocol always defaults to the OpenAI image-edit contract.
  const configuredEndpoint = String(config.endpoint || '').trim();
  const endpoint = configuredEndpoint && configuredEndpoint !== '/images/generations'
    ? configuredEndpoint
    : '/images/edits';
  const url = base + (endpoint.startsWith('/') ? endpoint : `/${endpoint}`);
  const boundary = `localminidrama_${crypto.randomBytes(12).toString('hex')}`;
  const textField = (name, value) => Buffer.from(
    `--${boundary}\r\nContent-Disposition: form-data; name="${name}"\r\n\r\n${value}\r\n`,
    'utf8'
  );
  const imageBuffer = fs.readFileSync(reference.filePath);
  const imageHeader = Buffer.from(
    `--${boundary}\r\nContent-Disposition: form-data; name="image"; filename="${reference.filename}"\r\nContent-Type: ${reference.mimeType}\r\n\r\n`,
    'utf8'
  );
  const parts = [
    textField('model', opts.model || 'gpt-image-2'),
    textField('prompt', opts.prompt || ''),
    textField('n', '1'),
    ...(opts.size ? [textField('size', opts.size)] : []),
    ...(opts.quality ? [textField('quality', opts.quality)] : []),
    imageHeader,
    imageBuffer,
    Buffer.from(`\r\n--${boundary}--\r\n`, 'utf8'),
  ];

  log.info('[私密直传图生] 提交 YYAPI 图片编辑请求', {
    endpoint_host: new URL(url).host,
    endpoint_path: new URL(url).pathname,
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    const message = providerErrorMessage(response.raw, response.statusCode);
    log.warn('[私密直传图生] YYAPI 返回失败', { status: response.statusCode, endpoint_host: new URL(url).host, message });
    return { error: `私密直传图片编辑失败: ${message}` };
  }
  let data;
  try { data = JSON.parse(response.raw); } catch (_) { return { error: '私密直传图片编辑返回格式异常' }; }
  const imageUrl = imageUrlFromOpenAIResponse(data);
  if (!imageUrl) return { error: '私密直传图片编辑未返回图片' };
  return { image_url: imageUrl, private_direct: true };
}
...
async function callImageApi(db, log, opts) {
  ...
  if (protocol === 'openai_image_edit_direct') {
    return callOpenAIImageEditDirect(config, log, {
  ...
  const isAgnesImage = protocol === 'agnes_image' || (provider === 'agnes' && /^agnes-image-/i.test(model));
  ...
  const agnesSize = isAgnesImage ? agnesImageRequestSize(effectiveSize) : null;

  const body = {
    model,
    prompt: effectivePrompt,
    // doubao-seedream API 不使用 n，其他 OpenAI 兼容接口保留
    ...(!isSeedream && !isAgnesImage ? { n: 1 } : {}),
    ...(isAgnesImage ? agnesSize : (effectiveSize ? { size: effectiveSize } : {})),
    ...(quality && !isAgnesImage ? { quality } : {}),
    ...
    ...(mergedNegativePrompt && !isAgnesImage ? { negative_prompt: mergedNegativePrompt } : {}),
    // 参考图字段：volcengine doubao-seedream API 规范使用 image（数组），见官方文档
    ...(resolvedRefs.length > 0 && !isAgnesImage ? { image: resolvedRefs } : {}),
    ...(isAgnesImage ? {
      extra_body: {
    ...
    try {
      ...
      if (sceneIdNum != null) {
        try {
          ...
          if (oldPath && !extras.includes(oldPath)) extras.push(oldPath);
          const extraJson = extras.length ? JSON.stringify(extras) : null;
          db.prepare("UPDATE scenes SET image_url = ?, local_path = ?, extra_images = ?, status = 'completed', error_msg = NULL, updated_at = ? WHERE id = ?").run(
            result.image_url,
            localPath,
            extraJson,
            now2,
          ...
          if ((e.message || '').includes('local_path') || (e.message || '').includes('extra_images')) {
            db.prepare("UPDATE scenes SET image_url = ?, status = 'completed', error_msg = NULL, updated_at = ? WHERE id = ?").run(result.image_url, now2, sceneIdNum);
          } else {
            throw e;
          }
        }
...
module.exports = {
  ...
  getProxyCache,
  setProxyCache,
  // Exported for focused privacy-routing tests. Do not call this without the
  // explicit openai_image_edit_direct protocol gate above.
  callOpenAIImageEditDirect,
  resolvePrivateDirectReference,
  agnesImageRequestSize,
  PRIVATE_FACTORY_ROOT,
};
