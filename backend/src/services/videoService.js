...
function resolveRemoteVideoUrl(videoUrl, fallbackError, downloadHeaders = null) {
  if (videoUrl && videoClient.isPlausibleHttpVideoUrl(videoUrl)) {
    return { ok: true, video_url: String(videoUrl).trim(), download_headers: downloadHeaders || null };
  }
  if (videoUrl) {
 ...
 */
async function downloadVideoToLocal(storagePath, videoUrl, videoGenId, log, projectSubdir = null, downloadHeaders = null) {
  ...
  try {
    ...
    const res = await fetch(videoUrl, { method: 'GET', headers: downloadHeaders || undefined });
    if (!res.ok) {
...
function normalizeVideoFileToTargetPixels(absPath, tw, th, log, videoGenId) {
  ...
  const baseArgs = ['-y', '-i', absPath, '-vf', vf, ...h264Args({ preset: 'fast', crf: 23 }), '-pix_fmt', 'yuv420p', '-movflags', '+faststart'];
  ...
  try {
    ...
    if (result.actual_model && !customerModelPolicy.customerMode()) {
      try {
        ...
        if (row.storyboard_id) {
          db.prepare('UPDATE storyboards SET actual_video_model = ?, updated_at = ? WHERE id = ?').run(
            result.actual_model, now2, row.storyboard_id
          );
        }
      } catch (error) {
        log.warn('Could not persist resolved Agnes model', { video_gen_id: videoGenId, error: error.message });
      }
    }
    const directVideo = resolveRemoteVideoUrl(result.video_url, result.error, result.download_headers);
    if (directVideo.ok) {
      let localPath = null;
      try {
        const loadConfig = require('../config').loadConfig;
        ...
        localPath = await downloadVideoToLocal(storagePath, directVideo.video_url, videoGenId, log, projectSubdir, directVideo.download_headers);
    ...
    if (result.task_id) {
      ...
      const polledVideo = resolveRemoteVideoUrl(pollResult.video_url, pollResult.error, pollResult.download_headers);
      if (polledVideo.ok) {
        let localPath = null;
        try {
          const loadConfig = require('../config').loadConfig;
          ...
          const storagePath = path.isAbsolute(cfg.storage?.local_path)
            ...
            : path.join(process.cwd(), cfg.storage?.local_path || './data/storage');
          const projectSubdir = storageLayout.getProjectStorageSubdir(db, row.drama_id);
          localPath = await downloadVideoToLocal(storagePath, polledVideo.video_url, videoGenId, log, projectSubdir, polledVideo.download_headers);
          maybeNormalizeVideoAfterDownload(storagePath, localPath, rowForAspect, videoGenId, log);
        } catch (_) {}
        try {
          db.prepare(
