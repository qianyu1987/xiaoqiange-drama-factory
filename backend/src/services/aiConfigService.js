...
function createConfig(db, log, req) {
  ...
  if (!endpoint && req.api_protocol === 'openai_image_edit_direct') {
...
async function testConnection(opts) {
  ...
  const protocol = (opts.api_protocol || '').toLowerCase();
  ...
  if (protocol === 'agnes_video') {
    ...
    if (!res.ok || !data) {
      const fallback = data
        ? `Agnes 连接失败 (${res.status})`
        : 'Agnes 接口返回非 JSON，请检查 Base URL';
  ...
  // HHTC video generation has its own job API. Probe its configuration route
  // rather than a chat endpoint, so testing never creates a billable video.
  if (protocol === 'hhtc_video_generation') {
    const root = base.replace(/\/v1$/i, '');
    const res = await fetch(root + '/pay/api/video-generation/config', {
      method: 'GET',
      headers: { Authorization: 'Bearer ' + opts.api_key },
    });
    const text = await res.text();
    let data = null;
    try { data = text ? JSON.parse(text) : null; } catch (_) {}
    if (!res.ok || !data?.success || !data?.data?.policy) {
      const fallback = data
        ? `HHTC 视频连接失败 (${res.status})`
        : 'HHTC 视频接口返回非 JSON，请检查公网反向代理';
      const message = data?.error?.message || data?.error || data?.message || fallback;
      throw new Error(String(message).slice(0, 500));
    }
    return;
  ...
  // The private direct image-edit route sends no user image during testing.
  // A 400/422 validates that YYAPI accepted the multipart contract; 401/403
  // remains a real credential failure and 404/405 flags a wrong endpoint.
  if (protocol === 'openai_image_edit_direct') {
    const path = endpoint && endpoint !== '/images/generations' ? endpoint : '/images/edits';
    const url = base + (path.startsWith('/') ? path : '/' + path);
    const form = new FormData();
    form.append('model', model || 'gpt-image-2');
    form.append('prompt', 'connectivity probe');
    ...
    if (res.status === 401 || res.status === 403) {
      throw new Error(`API Key 无效 (${res.status})`);
    }
    if (res.status === 404 || res.status === 405) {
      throw new Error(`私密图生直连端点不可用 (${res.status})`);
    }
    if (!res.ok && res.status !== 400 && res.status !== 422) {
      throw new Error(`私密图生直连探针失败 (${res.status})`);
    }
    return;
  }
  ...
  if (treatAsImage) {
    // A connection test must not generate a billable placeholder image.
    // OpenAI-compatible gateways, including HHTC, expose /models for this
    // exact credential-and-network probe.
    const url = base + '/models';
    console.log('[testConnection] 图片服务无扣费探针', { url, serviceType, model });
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error('图片连接探针超时（未发起生图请求）');
      }
      throw error;
    } finally {
