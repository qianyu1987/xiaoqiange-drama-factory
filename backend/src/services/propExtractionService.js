  ...
  try {
    ...
    const existingProps = db.prepare(
      `SELECT DISTINCT p.* FROM props p
       ...
       ORDER BY p.id ASC`
    ).all(Number(episodeId), Number(episodeId));
    if (existingProps.length > 0) {
      log.warn('Prop extraction used approved catalog fallback', {
        episode_id: Number(episodeId),
        prop_count: existingProps.length,
        provider_error: String(err.message || '').slice(0, 200),
      });
      taskService.updateTaskStatus(db, taskId, 'completed', 100, '文本模型不可用，已保留本集已锁定道具');
      taskService.updateTaskResult(db, taskId, {
        props: existingProps,
        count: existingProps.length,
        episode_id: Number(episodeId),
        drama_id: episode.drama_id,
        fallback: 'approved_catalog',
      });
      return;
    }
