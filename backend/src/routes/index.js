...
const pipelineRoutes = require('./pipelines');
const publishPackageRoutes = require('./publishPackages');
...
function setupRouter(cfg, db, log) {
  ...
  const product = productRoutes(db, cfg, log);
  const pipelines = pipelineRoutes(db, log);
  const publishPackages = publishPackageRoutes(db, cfg, log);
  ...
  // ---------- product / onboarding（订阅保护中间件之前） ----------
  r.get('/product/status', product.status);
  r.post('/product/onboarding', product.onboarding);
  r.post('/product/pair', product.pair);
  r.post('/product/activate', product.pair);
  r.get('/product/self-test', product.selfTest);
  r.get('/product/cloud-jobs/pending', product.pendingCloudJob);
  r.post('/product/cloud-jobs/:id/acknowledge', product.acknowledgeCloudJob);
  ...
  // ---------- persistent pipelines / publish packages ----------
  r.get('/pipeline-runs', pipelines.list);
  r.post('/pipeline-runs', pipelines.create);
  r.get('/pipeline-runs/:id', pipelines.get);
  r.post('/pipeline-runs/:id/commands', pipelines.command);
  r.put('/pipeline-runs/:id/steps/:step_key', pipelines.updateStep);
  r.get('/publish-packages/:episode_id', publishPackages.get);
  r.post('/publish-packages/:episode_id', publishPackages.create);
  r.get('/publish-packages/:episode_id/download', publishPackages.download);
