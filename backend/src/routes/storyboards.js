...
const { generateTextWithFallback } = require('../services/textModelFallbackService');
    ...
    polishPrompt: async (req, res) => {
      try {
        ...
        try {
          ...
          const drama = dramaId
            ? db.prepare('SELECT metadata FROM dramas WHERE id = ? AND deleted_at IS NULL').get(dramaId)
            : null;
          try { dramaMetadata = JSON.parse(drama?.metadata || '{}'); } catch (_) {}
        } catch (_) {}
        ...
        const promptModels = requestedModel
          ? [requestedModel]
          : Array.isArray(dramaMetadata.image_prompt_models)
            ? dramaMetadata.image_prompt_models
            : [];
        const generated = await generateTextWithFallback(db, log, {
          models: promptModels,
          userPrompt: userPromptLines.join('\n'),
          systemPrompt: promptI18n.getImagePolishPrompt(),
          options: { scene_key: 'image_polish', max_tokens: 300, temperature: 0.3 },
        });
        const polishedPrompt = generated.text;
        ...
        db.prepare('UPDATE storyboards SET polished_prompt = ?, image_prompt_model = ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL').run(
          polished, generated.model, nowIso, sbId
        );
        ...
        response.success(res, {
          ...
          image_prompt_model: generated.model,
