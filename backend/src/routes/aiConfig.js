...
function publicConfig(config) {
  ...
  return {
    ...
    base_url: customerMode
      ? (config.provider && String(config.provider).startsWith('hhtc') ? '由小钱哥短剧工厂托管' : '')
      : config.base_url,
  };
}
...
function create(db, log, cfg) {
  return (req, res) => {
    if (productService.flavor() === 'customer') {
      return response.forbidden(res, '客户版 AI 渠道由小钱哥短剧工厂统一托管');
    }
...
function update(db, log, cfg) {
  return (req, res) => {
    if (productService.flavor() === 'customer') {
      return response.forbidden(res, '客户版不允许修改 AI 渠道');
    }
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return response.badRequest(res, '无效的配置ID');
...
function remove(db, log, cfg) {
  return (req, res) => {
    if (productService.flavor() === 'customer') {
      return response.forbidden(res, '客户版不允许删除 AI 渠道');
    }
    if (aiConfigService.getVendorLockStatus(cfg).enabled) {
...
function testConnection(db, log) {
  return async (req, res) => {
    if (productService.flavor() === 'customer') {
      return response.forbidden(res, '客户版连接状态请在账号页查看');
    }
    let body = req.body || {};
    if (body.config_id && !String(body.api_key || '').trim()) {
      ...
      body = { ...stored, ...body, api_key: stored.api_key };
    }
